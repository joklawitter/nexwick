...
fn test_skip_whitespace() {
    let mut parser = ByteParser::from_str(" \r  \t\n \t x y");
...
fn test_skip_comment() {
    let mut parser = ByteParser::from_str("Tree tiny = [Following tree is tiny] ((A:1,B:1):1,C:2)");
...
fn test_consume_until_inclusive() {
    let mut parser = ByteParser::from_str("consume a CAN of beans");
...
fn test_consume_until_any_inclusive() {
    let mut parser = ByteParser::from_str("yummy! eat Apples\n");
    ...
    let mut parser = ByteParser::from_str("leek soup, hmm");
    let found = parser.consume_until_any(&targets, Inclusive);
...
fn test_consume_until_any_exclusive() {
    let mut parser = ByteParser::from_str("find yet do not eat the Banana!");
...
#[test]
fn test_is_eof() {
    let mut parser = ByteParser::from_str("... happily ever after!");
...
#[test]
fn test_position() {
    let mut parser = ByteParser::from_str("Where are we?");
...
fn test_parse_unquoted_label() {
    let mut parser = ByteParser::from_str("Scarabaeus:0.5");
    let delimiters = b"(),:; \t\n\r";
...
fn test_parse_quoted_label() {
    let mut parser = ByteParser::from_str("'Scarabaeus viettei':0.5");
...
fn test_parse_label_chooses_quoted() {
    let mut parser = ByteParser::from_str(" 'Quoted label' ");
    let delimiters = b"(),:; \t\n\r";
...
fn test_parse_label_chooses_unquoted() {
    let mut parser = ByteParser::from_str("  UnquotedLabel:");
    let delimiters = b"(),:; \t\n\r";
    let label = parser.parse_label(delimiters).unwrap();
...
fn test_slice_from_exclusive_at_eof() {
    let mut parser = ByteParser::from_str("HELLO");
    let pos = parser.position();
...
#[test]
fn test_get_context() {
    let mut parser = ByteParser::from_str("Hello World!");
